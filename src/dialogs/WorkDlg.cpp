...
void CWorkDlg::UpdateTotalTimer()
{
    ...
    if (m_ElapsedTimeTotal <= 59)
    {
        _stprintf(strTime, _T("%s 00:00:%02u\0"),
            theLangManager.HaveLangStrings() ? (LPCTSTR)theLangManager.GetLangString(0x00A01006) : _T("Total elapsed time:"),
...
void CWorkDlg::UpdateFileTimer()
{
    ...
    {
        _stprintf(strTime, _T("%s 00:00:%02u\0"),
            theLangManager.HaveLangStrings() ? (LPCTSTR)theLangManager.GetLangString(0x00A01005) : _T("Elapsed time:"),
...
void CWorkDlg::CreateWorker()
{
    ...
    if (this->hThread == NULL)
    {
        ...
        this->MessageBox(theLangManager.HaveLangStrings() ? theLangManager.GetLangString(0x00A0100B) : _T("Failed to create worker thread!"),
            theLangManager.HaveLangStrings() ? theLangManager.GetLangString(0x00A0100A) : _T("Fatal Error"),
            MB_OK | MB_ICONERROR);
    }
}
...
void CWorkDlg::InitLang()
{
    ...
    {
        this->SetWindowText(theLangManager.GetLangString(0x00A01001));
        this->GetDlgItem(IDCANCEL)->SetWindowText(theLangManager.GetLangString(0x00A01002));
        this->GetDlgItem(IDC_STATIC_ENCODER_LABEL)->SetWindowText(theLangManager.GetLangString(0x00A01007));
        this->GetDlgItem(IDC_STATIC_READS_LABEL)->SetWindowText(theLangManager.GetLangString(0x00A01008));
        this->GetDlgItem(IDC_STATIC_WRITES_LABEL)->SetWindowText(theLangManager.GetLangString(0x00A01009));
    }
}
